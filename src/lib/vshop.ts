...
const BASE_URL =
  process.env.VSHOP_BASE_URL?.replace(/\/$/, "") || "https://vshop.bz";
...
export async function getVshopBuildId(): Promise<string> {
  ...
  if (cachedBuildId && Date.now() - cachedBuildId.ts < 10 * 60 * 1000)
...
export function mapSearchItemToCatalog(it: VShopSearchItem): CatalogItem {
  return {
    ...
    originalPrice: it.original_price
      ? toMoney(it.original_price, parseMoneyValue(it.original_price))
      : null,
...
export function mapItemToDetail(
  ...
  return {
    ...
    originalPrice: data.original_price
      ? toMoney(data.original_price, parseMoneyValue(data.original_price))
      : null,
    ...
    // Variation support
    variation_id: (data as any).variation_id,
    variation: (data as any).variation,
    variation_array: (data as any).variation_array,
    has_variation_color_url: (data as any).has_variation_color_url,
    variation2_id: (data as any).variation2_id,
    variation2: (data as any).variation2,
    variation2_array: (data as any).variation2_array,
  };
}

export async function vshopItem(
  id: number,
  variationId?: number,
  variation2Id?: number,
): Promise<ItemDetail> {
  ...
  // Build query string with variation parameters
  const params = new URLSearchParams();
  params.set("id", String(id));
  if (variationId) params.set("variation_id", String(variationId));
  if (variation2Id) params.set("variation2_id", String(variation2Id));

  const url = `${BASE_URL}/_next/data/${buildId}/item/${id}.json?${params.toString()}`;
  console.log("🔍 VShop API call:", { id, variationId, variation2Id, url });
  ...
  console.log("✅ VShop response:", {
    id: data.id,
    variationId: (data as any).variation_id,
    variation2Id: (data as any).variation2_id,
  });
